...
sub close {
    ...
    my $ok = 1;
    if(defined $mode) {
        my $temp2 = File::Temp->new(UNLINK => 0, DIR => $store->_overlay);
        close $temp2;
        ...
        unless($store->_verify_blob($blob, $temp2->filename)) {
